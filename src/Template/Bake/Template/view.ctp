...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Alterar <%= $singularHumanName %>'), ['action' => 'edit', <%= $pk %>]) ?> </li>
        <li><?= $this->Form->postLink(__('Excluir <%= $singularHumanName %>'), ['action' => 'delete', <%= $pk %>], ['confirm' => __('Você tem certeza que deseja excluir # {0}?', <%= $pk %>)]) ?> </li>
...
<% endforeach; %>
            ...
            <%- $otherPk = "\${$otherSingularVar}->{$details['primaryKey'][0]}"; %>
                <td class="actions">
                    <?= $this->Html->link(__('Visualizar'), ['controller' => '<%= $details['controller'] %>', 'action' => 'view', <%= $otherPk %>]) %>
                    <?= $this->Html->link(__('Alterar'), ['controller' => '<%= $details['controller'] %>', 'action' => 'edit', <%= $otherPk %>]) %>
                    <?= $this->Form->postLink(__('Excluir'), ['controller' => '<%= $details['controller'] %>', 'action' => 'delete', <%= $otherPk %>], ['confirm' => __('Você tem certeza que deseja excluir # {0}?', <%= $otherPk %>)]) %>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
